 ...
 * a SAML Service Provider is being configured
...
// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Skipping nonce verification since we are getting the protocol type from the parameter.
if ( isset( $_GET['var'] ) ) {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Skipping nonce verification since we are getting the protocol type from the parameter.
	$var = esc_attr( sanitize_text_field( wp_unslash( $_GET['var'] ) ) );
} else {
	$var = '';
}
...
if ( ! empty( $sp ) || $sp_exists ) {
	...
	echo '<input type="hidden" name="service_provider" value="' . esc_attr( isset( $sp ) && ! empty( $sp ) ? $sp->id : '' ) . '" />
	...
	echo '<ul class="mo-idp-switch-tab mo-idp-text-center" >
		...
		echo '<li class="" onclick="moidpautohandler()" id="mo-idp-auto-upload">
								<a style="position:relative;top: -0.5rem !important;" class="mo-idp-upload-data-anchor" >
									<span class="mo-idp-enter-data">Upload SP Metadata</span>
								</a>
							</li>';
	echo '</ul>	
	...
	echo '
				...
				echo ' </tr>
						...
						echo '<input  type="text"
													...
													value="' . ( ! empty( $sp ) ? esc_attr( $relay_state ) : '' ) . '"
													/>';
					echo '</td>
									</tr>
									<tr><td>&nbsp;</td></tr>
	...
	echo '<img class="mo-idp-advt-premium-feature-lock" src="' . esc_url( MSI_LOCK ) . '"/>';
